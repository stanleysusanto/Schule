...
public class Util {
	...
	public static List csvToList(String csv) {
	 ...
	 public static String getActualTime(int hour, int minute) {
		 if (RemindMe.is24Hours()) return DbHelper2.getTimeStr(hour, minute);
		 
		 if (hour < 12) return DbHelper2.getTimeStr(hour, minute) + " AM";
		 else if (hour == 12) return DbHelper2.getTimeStr(12, minute) + " PM";
		 else return DbHelper2.getTimeStr(hour-12, minute) + " PM";
	 }
	...
	public static String toPersistentDate(String txt, SimpleDateFormat sdf) {
		try {
			return DbHelper2.sdf.format(sdf.parse(txt));
